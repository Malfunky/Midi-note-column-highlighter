scale_major = "102034050607" 
scale_minor = "102304050607"

major_chord =  "100030050000" 
minor_chord =  "100300050000"
key = 0  
...
-- Root note of the chord should be written with uppercase letter.
-- like this .. A#m , Bm , Cb 
-- no support yet for 7ths, 6ths, 9,11,13s.. 
-- this script is unfinished and relys on an active open midi editor
function readmarker(chord) 
  scaletable = {["C"] = 0, ["D"] = 2, ["E"]= 4 , ["F"]=5,["G"] = 7, ["A"] = 9 , ["B"] = 11}
  local a=string.match(chord,"^[ABCDEFGH]")
  local b=string.match(chord,"[#b]")
  local c=string.match(chord,"m") 
  if a then 
    key  = scaletable[a]
    if b then 
      if b=="b" then 
...
function main()
  ...
  if ok then -- "ok" makes sure there is enough info or good enough reason to draw.
    ...
    find_markers()   
    if startQN ~= startQN_ then unlinkBitmap();update=true end 
    if endQN == nil then unlinkBitmap();update = false end 

    readmarker(markername) -- this function decides the "key" and "chordtype" value by parsing/analysing the "markername" string.
    if chordtype == "minor" then
       scale = minor_chord end 
    if chordtype == "major" then 
       scale = major_chord 
    end 
    if markername=="" or markername ==nil then unlinkBitmap();ok= false end
  end
